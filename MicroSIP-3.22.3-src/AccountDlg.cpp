...
void AccountDlg::ApplyDefaultsAndLock()
{
    ...
    ((CEdit*)GetDlgItem(IDC_EDIT_DOMAIN))->SetWindowText(_T(_GLOBAL_ACC_DOMAIN));

    // Nome de Exibicao segue o Usuario (ramal)
    ((CEdit*)GetDlgItem(IDC_EDIT_DISPLAYNAME))->SetWindowText(m_Account.username);

    str.Format(_T("%d"), (int)_GLOBAL_ACC_REGISTER_REFRESH);
    ((CEdit*)GetDlgItem(IDC_ACCOUNT_REGISTER_REFRESH))->SetWindowText(str);
    str.Format(_T("%d"), (int)_GLOBAL_ACC_KEEP_ALIVE);
    ((CEdit*)GetDlgItem(IDC_ACCOUNT_KEEP_ALIVE))->SetWindowText(str);

    // Combos fixos: SRTP Desabilitado, Transporte UDP+TCP, Endereco Publico Auto
    ((CComboBox*)GetDlgItem(IDC_SRTP))->SetCurSel(0);
    ((CComboBox*)GetDlgItem(IDC_TRANSPORT))->SetCurSel(2); // UDP+TCP
    ((CComboBox*)GetDlgItem(IDC_PUBLIC_ADDR))->SetCurSel(0); // Auto

    // Checkboxes fixos
    ((CButton*)GetDlgItem(IDC_PUBLISH))->SetCheck(_GLOBAL_ACC_PUBLISH ? BST_CHECKED : BST_UNCHECKED);
    ...
    ((CEdit*)GetDlgItem(IDC_EDIT_PROXY))->SetReadOnly(TRUE);
    ((CEdit*)GetDlgItem(IDC_EDIT_DOMAIN))->SetReadOnly(TRUE);
    ((CEdit*)GetDlgItem(IDC_EDIT_DISPLAYNAME))->SetReadOnly(TRUE);
    ((CEdit*)GetDlgItem(IDC_EDIT_VOICEMAIL))->SetReadOnly(TRUE);
    ((CEdit*)GetDlgItem(IDC_ACCOUNT_DIALING_PREFIX))->SetReadOnly(TRUE);
    ((CEdit*)GetDlgItem(IDC_ACCOUNT_DIAL_PLAN))->SetReadOnly(TRUE);
    ((CEdit*)GetDlgItem(IDC_ACCOUNT_REGISTER_REFRESH))->SetReadOnly(TRUE);
    ((CEdit*)GetDlgItem(IDC_ACCOUNT_KEEP_ALIVE))->SetReadOnly(TRUE);

    // Combos e checkboxes desabilitados
    GetDlgItem(IDC_ACCOUNT_HIDE_CID)->EnableWindow(FALSE);
    GetDlgItem(IDC_SRTP)->EnableWindow(FALSE);
    GetDlgItem(IDC_TRANSPORT)->EnableWindow(FALSE);
    GetDlgItem(IDC_PUBLIC_ADDR)->EnableWindow(FALSE);
    GetDlgItem(IDC_PUBLISH)->EnableWindow(FALSE);
    GetDlgItem(IDC_REWRITE)->EnableWindow(FALSE);
    GetDlgItem(IDC_ICE)->EnableWindow(FALSE);
    GetDlgItem(IDC_SESSION_TIMER)->EnableWindow(FALSE);
}
